...
四项施政数值触及 0 时会先记录对应的阈值惩罚，随后触发任期终结事件：民生归零可能引发民变焚署，库银归零可能因亏空问斩，官声归零会被弹劾下狱，圣眷归零则奉旨拿问。终局会写入存档并锁定继续批文；达到 100 仍按原有奖励事件处理。

## 架构

浏览器只负责确定性规则、权威数值和最终成败；服务端 Agent 只改写官员的理解、盘算、行动口径和回文。模型不可更改 `fidelity`、`holdDays`、数值效果或结果成败。模型不可用、超时、输出越权或格式错误时，游戏自动采用确定性回退文本。

玩家点击批示时只会先记录朱批，不立即请求模型；点击“退堂·进入次日”后，客户端将当天所有待处理的官员步骤合并提交到 `/api/agents/propagate-batch`，由编排器一次生成整批下行叙事。这样可以避免逐件往返，同时保留无模型时的确定性回退。
