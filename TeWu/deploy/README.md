...
当前版本包含 `tewu-server` 服务端 Agent：执行官模式的候选人和潜伏者模式的审查官均可通过 LLM 生成对话。`agent-configmap.yaml` 只保存 llm-service 的入口与模型别名；调用令牌由 `vault/inventory/tewu-llm-token-externalsecret.yaml` 注入到 `tewu/llm-token` Secret。模型请求最多重试三次；三次失败后接口返回错误，前端只显示“模型正在忙，请稍后再提问”，不会生成本地替代台词。
