...
当前版本包含 `tewu-server` 服务端 Agent：执行官模式的候选人和潜伏者模式的审查官均可通过 LLM 生成对话。`agent-configmap.yaml` 保存非敏感 provider 参数；凭据由 `vault/inventory/tewu-agent-externalsecret.yaml` 从独立 Vault 路径 `secret/data/tewu/agent` 注入到 `tewu/tewu-agent` Secret。模型请求最多重试三次；三次失败后接口返回错误，前端只显示“模型正在忙，请稍后再提问”，不会生成本地替代台词。
