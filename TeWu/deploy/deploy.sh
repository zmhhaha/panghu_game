...
kubectl wait --for=condition=Ready externalsecret/tewu-agent -n "${namespace}" --timeout=90s
kubectl wait --for=condition=Ready externalsecret/tewu-database -n "${namespace}" --timeout=90s
