...
一款以历史参考、混合时空和多 Agent 对话为核心的身份辨识游戏。玩家选择一个真实存在过的特务机构，扮演该机构的执行官，同时面对十名可自由盘问的 NPC。执行官模式不限制对单个 NPC 或整局的提问次数，玩家通过反复切换人物、比较口供和追查多组熟人关系，自行决定何时提交任意数量的扣留对象，最终以目标召回率与普通人正确放行率共同决定行动结果。
...
- 每局在十人中随机生成 2 至 4 名目标；简报只公开这个情报范围，不公开本局确切人数。
- 玩家提交名单后，名单中的目标计为命中，名单中的普通来客计为误捕；未入选的目标计为漏网，未入选的普通来客计为正确放行。
- 平衡分 =（目标命中数 / 目标总数 + 普通人正确放行数 / 普通人总数）/ 2 × 100%。全部放行或全部扣留都只能获得 50 分，不能利用人数比例取得及格结果。
- 十人逐项准确率仍作为次要统计展示，但评级只使用公开公式计算的平衡分。名单可以为空，也可以包含多人，不限制固定扣留人数。
- 基础评级：`S`（100%）、`A`（90% 及以上）、`B`（80% 及以上）、`C`（70% 及以上）、`D`（60% 及以上）、`E`（低于 60%）。
- 误捕和漏网分别统计，作为结局文本和机构评价的依据；结算同时展示目标命中率、普通人正确放行率和平衡分。
...
主控只接受限定范围内的数值和档案中存在的事实 ID，再与少量确定性规则合成当轮可疑度变化。确定性规则只处理回答过短、明确拒答、精确档案锚点命中等可以机械确认的情况；未命中某个词不能单独增加可疑度。Agent 评价异常或网络失败时不生成本地角色台词、不写入口供，客户端最多提示“模型正在忙，请稍后再提问”；不允许一次模型输出直接决定整局胜负。
...
- 维护玩家当前选中的 NPC、十份独立对话记录、全局事实账本、待提交名单、平衡分和事件日志，不使用执行官模式的轮数门槛。
...
- 只能依据角色档案和当时可知的历史信息作答，不能读取其他 NPC、平衡分或主控判定。
...
关系组不再把“时间相邻”“路线交叉”直接作为完整可见台词。主控为每组生成一件可追溯的共同事件，事件至少包含：

- `eventId`、发生地点、时间窗口、一个可见物件或编号、一个可被核验的记录来源，以及事件中各步骤的固定顺序。
- `memberViews`：每名成员亲眼看见、听说、记住或刻意忽略的不同片段；成员不知道的字段必须明确写入档案。
- `anchorFacts`：至少两个能被不同成员独立提到的锚点，例如同一站台的钟点与同一只箱子的封签；锚点的表达可以不同，但不能临场改变。
- `concealedContext`：潜伏者试图掩盖的目的、联系人或交接动作；普通成员也可以只知道表面工作，不能因为“不完整”自动成为目标。

共同事件的角色回答必须围绕具体场景自然组织语言，不得把 `memberViews` 中的内部字段或“我作为某职业只知道……”之类模板句直接展示给玩家。一个成员可以说出时间但记不住人名，另一个成员可以认出物件却不知道用途，第三个成员只听到传闻；差异来自知识范围，而不是随机矛盾。玩家引用原话后，NPC 只能补充自己档案中已经存在的片段，并优先使用姓名、地点、时间、编号和动作等可比较锚点。

每局至少有一组关系事件与潜伏目标有关，至少一组只涉及普通人的工作、邻里或生活违规。关系事件在结算前不标记为“任务关系”，玩家必须通过多人的具体回答判断它究竟是正常交集、灰色隐瞒还是潜伏任务。

### 8.2.2 真实模型失败处理

执行官和潜伏者的对话请求都只允许使用真实 Agent 返回。单次请求失败后由服务端 Agent 网关自动重试，最多三次；每次重试使用同一轮问题、同一份固定档案和同一段历史，不增加轮数、不写入口供、不改变事实账本。三次均失败时：

- 当前玩家问题保留在输入框，NPC 不新增回答、主张、观察或情绪记录。
- 对话区显示“模型正在忙，请稍后再提问”，允许玩家查看其他已完成的 NPC，但在该请求结束前不能提交名单或再次发送同一对象的问题。
- 不显示本地降级台词，不把失败伪装成角色回答；稍后重新提问时才重新发起真实请求。
- 服务端记录失败原因和重试次数，客户端只显示中性忙碌提示，避免把网络故障误认为人物破绽。

### 8.2.3 推理公平性契约

主控在调用角色 Agent 之前生成确定性的公平性档案，模型只能负责角色化表达，不能决定本局是否存在足够线索：

- 每名潜伏目标拥有一条 `fairnessClue`，包含公开掩护说法、与之不兼容的记录、核验来源、可触发主题和结案说明。玩家追问对应主题、引用同组成员或执行机构核验时，Agent 必须从这条固定线索中透露至少一个可比较锚点。
- 每名普通来客拥有一条 `closureClue`，包含表面异常及其可复核闭环。普通人可以紧张、记错无关细节或隐瞒生活违规，但连续追问后必须能够给出不依赖隐藏真值的解释和记录来源。
- 角色职业与目标身份每局随机组合，角色的 `tell` 和结案说明必须按本局真实身份重新生成。普通人即使沿用了角色池中原本偏可疑的职业，也只能把手续违规、私人秘密或记忆偏差解释为非潜伏证据，不能继承旧的定罪标签。
- 目标冲突必须能由本地知识、机构核验或另一名来客的口供独立发现，不能只写在结算档案中；任一目标都不得依靠语气、职业刻板印象或政治口号作为唯一识别依据。
- 机构核验对目标和普通人使用完全相同的中性格式，只并列核验类别、原始记录、已有口供和来源，不出现“目标”“异常闭环”“公平线索”等身份提示。核验类别可以引导玩家定向追问，避免为了寻找隐藏主题而对十人机械问完所有类别。
- 目标在关键主题被追问后必须提供预生成的不兼容事实，但仍会维护自己的掩护解释；Agent 不得主动说“我前后矛盾”“刚才说法对不上”“我撒了谎”或自报身份。服务端检测到这类替玩家下结论的回答时，将其视为模型失败并在三次上限内重试。
- 普通关系事件的实际成员必须全部是普通来客；只要组内存在目标，该事件就按目标关系生成，并为至少一名目标写入与其他成员观察不兼容的锚点。
- 政治立场只在与具体行动、组织记录或既往事件相连时具有判断价值。所有人都会给出符合高压环境的公开表态，口号本身既不能洗清嫌疑，也不能构成目标证据。
- 结算逐人引用实际的冲突或闭环、相关记录来源和玩家取得的原话；没有被玩家问出的线索可以在复盘中说明，但不得伪装成玩家本应已经知道的证据。

每局在十人中随机生成 2 至 4 名目标，简报向玩家公开这个范围。主控使用平衡分评价名单，使全部放行、全部扣留和只利用人数比例猜测都不能获得高评级。
...
- 完成主控状态机、隐藏真值、十人同时展示、自由切换、不限提问、统一扣留名单和平衡分结算。
...
- 每局目标数量在 2 至 4 人之间随机产生，简报公开范围但不泄露确切人数；全部放行或全部扣留的平衡分均为 50。
- 十个 NPC 的隐藏身份在开局时固定，刷新、重试或模型更换后不发生变化。
- 每名目标至少有一条能通过本地知识、机构核验或交叉口供发现的固定冲突；每名普通人至少有一条能闭环表面异常的可核验记录。
- 标记为普通关系的事件不得包含目标；目标关系中至少有一个预先生成、可被不同成员回答触发的锚点冲突。
...
- 结算准确展示正确判断、误捕、漏网、目标命中率、普通人正确放行率、平衡分和基于实际口供的逐人依据。
