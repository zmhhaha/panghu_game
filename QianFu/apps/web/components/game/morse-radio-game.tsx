...
export function MorseRadioGame({ challenge, selection, busy, onCancel, onSend }: {
  ...
  const complete = inputs.length >= expected.length;
    ...
    <main className="mx-auto max-w-5xl px-4 py-7 sm:px-6 sm:py-10">
      <div className="grid gap-6 md:grid-cols-[minmax(0,1fr)_260px]">
        <section className="min-w-0">
          ...
          <div className="mt-4 flex items-center justify-between text-xs"><span className={interference === "频率稳定" ? "text-muted" : "text-copper"}>{interference}</span><span className="text-muted">修正 {correctionCount}/{challenge.config.correctionAllowance}</span></div>
