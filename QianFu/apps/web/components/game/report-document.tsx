...
export function ReportDocument({ report, preview = false }: { report: CampaignReport; preview?: boolean }) {
  const fieldText = (intel: CampaignReport["intel"][number], fields: string[]) => fields.map((field) => intel.fieldLabels[field] ?? field).join("、");
  const scoreItems = [
