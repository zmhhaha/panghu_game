...
export function CoverIdentityPanel({ state, busy, onAction }: { state: PublicWorldState; busy: boolean; onAction: (action: GameAction) => void }) {
  ...
  const atOffice = state.currentLocationId === "archive-office";
  const workingHours = minuteOfDay(state.currentTime) >= 480 && minuteOfDay(state.currentTime) < 1020;
  ...
  return <section className="mt-5 border-t border-line pt-4">
    <div className="flex items-center justify-between gap-3"><p className="flex items-center gap-2 text-xs font-medium text-muted"><BriefcaseBusiness size={15} />公开身份：机要处档案员</p><span className={cover.workStatus === "unexcused_absence" ? "text-[10px] text-alert" : cover.workStatus === "working" ? "text-[10px] text-safe" : "text-[10px] text-muted"}>{statusLabels[cover.workStatus]}</span></div>
    ...
    {!atOffice ? <p className="mt-3 text-[10px] text-muted">需返回机要楼档案科办理工作或请假。</p> : !workingHours ? <p className="mt-3 text-[10px] text-muted">公开工作时段为 08:00 至 17:00。</p> : null}
