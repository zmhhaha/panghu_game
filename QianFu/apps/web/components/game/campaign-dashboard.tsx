    ...
    <div className="mx-auto max-w-7xl px-5 py-8 lg:px-8">
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        ...
        <aside className="h-fit border border-line bg-panel p-5">
          ...
          <div className="mt-3 space-y-2">{availableProfiles.map((profile) => { const Icon = profile.id === "travelling_merchant" ? Store : profile.id === "freelance_writer" ? PenLine : BriefcaseBusiness; const selected = profile.id === coverProfileId; const override = selectedCampaign?.coverProfileOverrides?.[profile.id]; const title = override?.title ?? profile.title; const summary = override?.summary ?? profile.summary; const routineLabel = override?.routineLabel ?? profile.routineLabel; return <button key={profile.id} onClick={() => setCoverProfileId(profile.id)} className={`w-full border p-3 text-left transition-colors ${selected ? "border-copper bg-copper/10" : "border-line hover:border-paper/40"}`}><span className="flex items-center gap-2 text-sm"><Icon size={15} className="text-copper" />{title}</span><span className="mt-1 block text-[11px] leading-5 text-muted">{summary}</span><span className="mt-1 block text-[10px] text-paper/70">约束：{routineLabel}</span></button>; })}</div>
