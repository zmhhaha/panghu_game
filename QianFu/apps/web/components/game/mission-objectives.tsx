...
export function MissionObjectives({ state, context }: { state: PublicWorldState; context: GameContext }) {
  return <section className="mt-7 border-t border-line pt-5">
    ...
    <div className="mt-4 space-y-4">{context.objectives.map((objective) => <article key={objective.id} className="border border-line bg-panel/40 p-4">
      ...
      <div className="mt-4 flex items-center justify-between border-t border-line pt-3 text-[10px] text-muted"><span>{objective.status === "locked" ? "完成当前任务后等待组织进一步指示" : objective.status === "completed" ? "所有结算条件已满足" : objective.status === "ready_to_transmit" ? "情报已备齐，可以安排传递" : objective.status === "overdue" ? "已超过截止时间" : "继续收集并核验情报"}</span>{objective.status !== "locked" && <span className={objective.remainingMinutes < 120 && objective.status !== "completed" ? "text-alert" : "text-muted"}><Clock3 size={11} className="mr-1 inline" />剩余 {formatRemaining(objective.remainingMinutes)}</span>}</div>
    </article>)}</div>
  </section>;
}
