...
export function MissionObjectives({ state, context }: { state: PublicWorldState; context: GameContext }) {
  return <section className="mt-7 border-t border-line pt-5">
    ...
    <div className="mt-4 space-y-4">{context.objectives.map((objective) => <article key={objective.id} className="border border-line bg-panel/40 p-4">
      ...
      <div className="mt-4 flex items-center justify-between border-t border-line pt-3 text-[10px] text-muted"><span>{objective.status === "locked" ? "等待前序任务结算后接收组织指示" : objective.status === "completed" ? "所有结算条件已满足" : objective.status === "failed" ? "本项任务已经结束，后续任务仍可继续" : objective.status === "ready_to_transmit" ? "情报已备齐，可以安排传递" : objective.status === "overdue" ? "已超过截止时间，等待任务结算" : "继续收集并核验情报"}</span>{objective.status !== "locked" && objective.status !== "failed" && <span className={objective.remainingMinutes < 120 && objective.status !== "completed" ? "text-alert" : "text-muted"}><Clock3 size={11} className="mr-1 inline" />剩余 {formatRemaining(objective.remainingMinutes)}</span>}</div>
    </article>)}</div>
  </section>;
}
...
function Status({ status }: { status: GameContext["objectives"][number]["status"] }) {
  if (status === "locked") return <span className="text-[10px] text-muted">尚未解锁</span>;
  if (status === "completed") return <span className="flex items-center gap-1 text-[10px] text-safe"><Check size={12} />已完成</span>;
  if (status === "failed") return <span className="flex items-center gap-1 text-[10px] text-alert"><CircleAlert size={12} />本项失败</span>;
