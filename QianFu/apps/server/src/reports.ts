...
export function buildCampaignReportBundle(
  ...
  const ownerReport: CampaignReport = {
    ...
    intel: visibleIntel.map((definition) => ({
      ...
      fieldLabels: { ...definition.fieldLabels },
...
function buildTimeline(campaign: CampaignDefinition, events: GameEvent[]): CampaignReportTimelineEntry[] {
  const important = events.filter((event) => [
    ...
    "character.recruitment_progress", "comrade.request_responded", "comrade.commitment_confirmed", "comrade.request_cancelled", "comrade.task_completed", "comrade.task_failed",
...
function describeEvent(campaign: CampaignDefinition, event: GameEvent): CampaignReportTimelineEntry {
  ...
  const details: Record<string, [string, string]> = {
    ...
    "comrade.request_responded": ["合作回应", String(payload.message ?? "网络成员回应了玩家的合作请求")],
    "comrade.commitment_confirmed": ["确认合作", "双方确认了本次行动的范围、条件和中止边界"],
    "comrade.request_cancelled": ["撤回合作", "玩家撤回了一项尚未结束的合作请求"],
    "comrade.task_completed": ["同志回报", String(payload.report ?? "一项合作行动已经完成")],
    "comrade.task_failed": ["合作受挫", String(payload.report ?? "一项合作行动未能完成")],
