...
export function createAgentProvider(): AgentProvider | null {
  switch ((process.env.PROVIDER ?? "fallback").toLowerCase()) {
    ...
    case "openai-compatible":
      return new OpenAiCompatibleProvider(process.env.OPENAI_BASE_URL ?? "https://api.openai.com/v1", process.env.OPENAI_API_KEY, process.env.OPENAI_MODEL ?? "gpt-4o-mini");
    case "anthropic":
      return new AnthropicProvider();
    case "deepseek":
      return new OpenAiCompatibleProvider(process.env.DEEPSEEK_BASE_URL ?? "https://api.deepseek.com", process.env.DEEPSEEK_API_KEY, process.env.DEEPSEEK_MODEL ?? "deepseek-chat");
    case "custom":
      return new OpenAiCompatibleProvider(process.env.CUSTOM_BASE_URL, process.env.CUSTOM_API_KEY, process.env.CUSTOM_MODEL ?? "qianfu-npc");
    default:
      return null;
  }
}
